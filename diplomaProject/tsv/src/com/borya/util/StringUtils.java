...
public class StringUtils {
	 ...
	 * @return
	 */
	public static String decorateIds(String ids){
	...
	/*
	 * public static boolean isEmpty(String str){ if(str == null ||
	 * "".equals(str)){ return false; } return true; }
	 */
	 ...
	 * original password.
	 * <p/>
	 * In Jive, every time a user loggers in, we simply take their plain text
		...
		synchronized (algorithm.intern()) {
			...
			if (digest == null) {
				try {
					...
					logger.error("Failed to load the " + algorithm
							+ " MessageDigest. "
	...
	public static String getStaffNosStr(List<String> list) {
		StringBuffer sb = new StringBuffer();
		if (list.size() != 0) {
			for (int i = 0; i < list.size(); i++) {
				sb.append("'" + String.valueOf(list.get(i)) + "',");
	 ...
	 * @throws Exception
	 */
	public static String removeSquareBrackets(String arrayStr) throws Exception {

		String aryStr = arrayStr;
		boolean startflag = aryStr.startsWith("[");
		boolean endflag = aryStr.endsWith("]");
		if (startflag) {
			aryStr = aryStr.substring(1);
		} else {
			throw new Exception();
		}
		if (endflag) {
			aryStr = aryStr.substring(0, aryStr.length() - 1);
		}
		return aryStr;
	}

	public static String replaces(String from, String to, String source) {
	...
	public static String dealJson(String source) {
		String removeleftbrackets = StringUtils.replaces("[", "", source);
		String removerightbrackets = StringUtils.replaces("]", "",
				removeleftbrackets);
		String finalResult = StringUtils.replaces("\"", "'",
				removerightbrackets);
		return finalResult;
	...
	public static String addapostrophes(String sourceStr) {
		if (StringUtils.isNULL(sourceStr)) {
			return "";
		} else {
			return "'" + StringUtils.replaces(",", "','", sourceStr) + "'";
		}
	}
	...
	public static List<Integer> transferStringToList(String str, String splitStr) {
